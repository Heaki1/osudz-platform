...
export function Leaderboard({ maps }: LeaderboardProps) {
  const [tab, setTab] = useState<'voters' | 'maps'>('voters');

  const topMaps = [...maps].sort((a, b) => b.votes - a.votes).slice(0, 8);
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 pb-16">
      ...
      {tab === 'voters' && (
        <div className="space-y-2">
            ...
            {[topVoters[1], topVoters[0], topVoters[2]].map((entry, i) => {
              ...
              return (
                <div key={entry.user} className="flex flex-col items-center">
                  <div className="w-10 h-10 rounded-full bg-slate-800 border-2 border-slate-600 flex items-center justify-center text-sm font-black text-white mb-1 uppercase">{entry.user[0]}</div>
          ...
          {topVoters.slice(3).map((entry) => {
            const bc = badgeColors[entry.badge];
            return (
              <div key={entry.user} className="flex items-center gap-3 rounded-xl p-3 border bg-slate-900/50 border-slate-800 hover:border-slate-600 transition-all">
                <span className="text-sm font-black text-slate-500 w-5 text-center font-mono">#{entry.rank}</span>
                <div className="w-8 h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-xs font-black text-slate-300 uppercase flex-shrink-0">{entry.user[0]}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold truncate text-white">{entry.user}</span>
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full border ${bc.text} ${bc.bg} ${bc.border}`}>{entry.badge}</span>
                  </div>
                  <div className="flex items-center gap-3 mt-0.5">
                    <span className="text-[10px] text-slate-500 font-mono">{entry.votes} votes</span>
                    <span className="text-[10px] text-slate-500 font-mono"><Flame className="w-2.5 h-2.5 inline text-orange-400 mr-0.5" />{entry.streak}d streak</span>
                  </div>
                </div>
      ...
      {tab === 'maps' && (
        <div className="space-y-2">
          {topMaps.length === 0 && <p className="text-sm text-slate-500 text-center py-8">No maps loaded yet.</p>}
          {topMaps.map((m, idx) => (
            <div key={m.id} className="flex items-center gap-3 bg-slate-900/50 border border-slate-800 hover:border-slate-600 rounded-xl p-3 transition-all">
              <div className="w-7 flex items-center justify-center flex-shrink-0">
                {idx < 3 ? rankIcons[idx] : <span className="text-sm font-black text-slate-600 font-mono">#{idx + 1}</span>}
              </div>
              <div className="flex-1 min-w-0">
              ...
              <div className="text-right flex-shrink-0">
                ...
                <div className="flex items-center gap-0.5 justify-end mt-0.5">
                  ...
                  <span className="text-[10px] text-slate-500 font-mono">{m.difficultyRating.toFixed(2)}</span>
                </div>
              </div>
            </div>
          ))}
