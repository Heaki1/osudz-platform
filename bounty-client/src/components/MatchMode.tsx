...
const STATS: { key: keyof BeatmapBounty; label: string }[] = [
  { key: 'difficultyRating', label: 'Difficulty' },
  { key: 'votes',            label: 'Votes'      },
  { key: 'bpm',              label: 'BPM'        },
  { key: 'approachRate',     label: 'AR'         },
  { key: 'hpDrain',          label: 'HP Drain'   },
];

function MiniCard({ map, selected, onClick, disabled }: { map: BeatmapBounty; selected: boolean; onClick: () => void; disabled?: boolean }) {
  ...
  return (
    <button type="button" onClick={onClick} disabled={disabled}
      className={`relative rounded-xl overflow-hidden border transition-all text-left w-full ${
        selected ? 'border-amber-400 shadow-lg shadow-amber-400/20 scale-[1.02]' : 'border-slate-700/60 hover:border-slate-500'
      ...
      <div className="relative h-20 bg-slate-900">
        ...
        <div className={`absolute top-0 left-0 right-0 py-1 px-2 flex items-center justify-between text-[10px] font-bold ${isHigh ? 'bg-rose-500 text-white' : 'bg-amber-400 text-slate-950'}`}>
          <div className="flex gap-0.5">{Array.from({ length: Math.min(5, Math.ceil(map.difficultyRating)) }).map((_, i) => <Star key={i} className="w-2.5 h-2.5 fill-current" />)}</div>
      ...
      {selected && <div className="absolute top-7 right-1.5 w-4 h-4 rounded-full bg-amber-400 flex items-center justify-center"><CheckCircle2 className="w-3 h-3 text-slate-950" /></div>}
    </button>
  );
}
...
export function MatchMode({ maps, onAwardPoints }: MatchModeProps) {
  ...
  const startBattle = () => { setPhase('battle'); setRevealed(false); setCountdown(3); setWinner(null); };
  const reset = () => { setPhase('pick'); setSelectedId(null); setRevealed(false); setCountdown(3); setWinner(null); setBattleStatIdx((i) => (i + 1) % STATS.length); };

  const pVal = playerCard ? (playerCard[battleStat.key] as number) : null;
  const oVal = opponentCard[battleStat.key] as number;

  if (maps.length === 0) {
    return <div className="text-center py-24 text-slate-400 px-6"><p>No maps loaded — match mode requires an active challenge.</p></div>;
  }
      ...
      {phase === 'pick' && (
        <>
          <div className="mb-6 flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-rose-500/20 border border-rose-500/50 flex items-center justify-center"><Swords className="w-5 h-5 text-rose-400" /></div>
            <div>
              <p className="text-base font-black text-white">Head-to-Head vs <span className="text-amber-400">{OPPONENT_NAME}</span></p>
              <p className="text-xs text-slate-400">Pick one card. Highest <span className="text-amber-400 font-bold">{battleStat.label}</span> wins this round.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6 sm:grid-cols-3">
            {maps.map((m) => <MiniCard key={m.id} map={m} selected={selectedId === m.id} onClick={() => setSelectedId(m.id)} />)}
          </div>
          <button type="button" disabled={!selectedId} onClick={startBattle}
            ...
            <Swords className="w-4 h-4" /> Play Card <ChevronRight className="w-4 h-4" />
          </button>
        </>
      )}
      ...
      {phase === 'battle' && playerCard && (
        <div className="flex flex-col items-center">
          ...
          <div className="w-full grid grid-cols-2 gap-4 mb-6">
            {[
              { card: playerCard, label: 'YOU',                  val: pVal, side: 'player'   },
              { card: opponentCard, label: OPPONENT_NAME.toUpperCase(), val: oVal, side: 'opponent' },
            ].map(({ card, label, val, side }) => (
              <div key={side} className={`rounded-2xl overflow-hidden border transition-all duration-500 ${
                revealed
                  ? winner === side ? 'border-emerald-400 shadow-lg shadow-emerald-400/20'
                  : winner === 'draw' ? 'border-amber-400'
                  : 'border-rose-500 opacity-70'
                  : 'border-slate-700'
              }`}>
                <div className="relative h-28 bg-slate-900">
                  {card.bannerUrl ? <img src={card.bannerUrl} alt={card.title} referrerPolicy="no-referrer" className="w-full h-full object-cover opacity-80" /> : <div className="w-full h-full bg-slate-800" />}
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0f172a]/90 to-transparent" />
                  <span className="absolute bottom-2 left-2 text-[10px] font-bold text-white bg-slate-950/70 px-1.5 py-0.5 rounded">{label}</span>
                </div>
                <div className="bg-[#0f172a] p-3">
                  <p className="text-xs font-bold text-white line-clamp-1 mb-1">{card.title}</p>
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-slate-400">{battleStat.label}</span>
                    <span className={`text-lg font-black font-mono ${
                      revealed && winner === side ? 'text-emerald-400'
                      : revealed && winner !== 'draw' ? 'text-rose-400'
                      : 'text-white'
                    }`}>
                      {revealed ? (val as number).toFixed((val as number) === Math.floor(val as number) ? 0 : 2) : '?'}
                    </span>
                  </div>
                </div>
              </div>
          ...
          {!revealed ? (
            <div className="text-center">
              <div className="w-20 h-20 rounded-full border-4 border-amber-400 flex items-center justify-center mb-3" style={{ transform: `scale(${1 + (3 - countdown) * 0.08})` }}>
            ...
            <div className="text-center">
              {winner === 'player' && <><Trophy className="w-10 h-10 text-yellow-400 mx-auto" /><p className="text-2xl font-black text-emerald-400 mt-2">You Win!</p><p className="text-sm text-slate-400">+{playerCard.bountyRewardPoints} pts awarded</p></>}
              {winner === 'opponent' && <><p className="text-2xl font-black text-rose-400">Opponent Wins</p><p className="text-sm text-slate-400">Better luck next round</p></>}
              {winner === 'draw' && <><p className="text-2xl font-black text-amber-400">Draw!</p><p className="text-sm text-slate-400">Evenly matched</p></>}
              <button type="button" onClick={reset} className="mt-5 px-6 py-2.5 rounded-xl bg-amber-400 hover:bg-amber-300 text-slate-950 font-black text-sm transition-all flex items-center gap-2 mx-auto active:scale-[0.98]">
